...
static SERVER: OnceCell<String> = OnceCell::new();

#[tokio::main]
async fn main() {
    let server = std::env::args().nth(1).expect("need url argument");
    SERVER.set(server.into()).expect("set static server");
...
async fn spawn_client(who: usize) {
    let ws_stream = match connect_async(SERVER.get().expect("get static server")).await {
        Ok((stream, response)) => {
